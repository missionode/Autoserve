...
const destructive = [
  ...
  /TRUNCATE\s+/i,
  /ALTER\s+TABLE.+DROP\s+COLUMN/is,
];
...
for (const directory of directories) {
  if (!/^\d{12,14}_[a-z0-9_]+$/.test(directory.name))
    throw new Error(`Invalid migration name: ${directory.name}`);
  ...
  if (destructive.some((pattern) => pattern.test(sql)))
    throw new Error(`Destructive migration requires explicit corrective review: ${directory.name}`);
}
